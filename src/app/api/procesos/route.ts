...
import { intConsultaNumeroRadicacion, intProceso } from "../procesos";
...
export async function GET ( request: Request ) {
    ...
    if ( llaveProceso ) {
        const procesos = data.find(
            ( procesos ) => procesos.llaveProceso === llaveProceso
        );
    ...
    const idProceso =
        searchParams.get( "idProceso" );
    if ( idProceso ) {
        const procesos = data.find(
            ( procesos ) => procesos.idProceso.toString() === idProceso
        ); return new NextResponse(
            JSON.stringify( procesos ?? null ),
            {
                status: 200,
...
const usersByLikes = data.map( ( item ) => {
    ...
    procesos[ item.procesos ] = item.likes;
